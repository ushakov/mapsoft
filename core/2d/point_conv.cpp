...
void AffConv::bck_recalc(){
  double D = k_frw[0] * k_frw[4] - k_frw[1] * k_frw[3];

  if (D==0) throw "AffConv: can't calculate matrix for backward conversion.";
...
AffConv::AffConv(): k_frw(6), k_bck(6){
  reset();
}

AffConv::AffConv(const std::map<dPoint, dPoint> & ref): k_frw(6), k_bck(6){
  set_from_ref(ref);
}

void
AffConv::set_from_ref(const std::map<dPoint, dPoint> & ref){
/*
  a x + b y + c = X
  d x + e y + f = Y
...
#define A7(x,y) a[(x)+(y)*7]
  ...
  if (mdiag (6, a) != 0) throw "AffConv: can't calculate conversion matrix.";
...
void
AffConv::scale(const double kx, const double ky){
