...
void
Rubber::rubber_draw_segment(const RubberSegment &s){
  int r,w,h,x,y;
...
void
Rubber::rubber_draw(const bool all){
  if (!rubber_gc) return;
...
void
Rubber::rubber_erase(const bool all){
  if (!rubber_gc) return;
...
void
Rubber::rubber_add(const RubberSegment & s){
  rubber_erase();
  rubber.push_back(s);
  rubber_draw();
}
...
void
Rubber::rubber_add(const iPoint & p1, const iPoint & p2, const rubbfl_t flags){
  rubber_add(RubberSegment(p1, p2, flags));
}

void
Rubber::rubber_add(const int x1, const int y1,
...
void
Rubber::rubber_add_src_sq(const iPoint & p, int size){
...
void
Rubber::rubber_add_dst_sq(int size){
  iPoint p1(-size,-size), p2(-size,size);
...
void
Rubber::rubber_add_diag(const iPoint & p){
  rubber_add(p, iPoint(0,0), RUBBFL_MOUSE_P2);
}

void
Rubber::rubber_add_rect(const iPoint & p){
