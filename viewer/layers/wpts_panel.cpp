...
void
WptLL::add_gobj (const boost::shared_ptr<GObjWPT> layer,
...
void
WptLL::remove_gobj (const GObjWPT * L){
...
void
WptLL::get_data(geo_data & world, bool visible) const {
  Gtk::TreeNodeChildren::const_iterator i;
  for (i  = store->children().begin();
       i != store->children().end(); i++){
...
int
WptLL::find_wpt(const iPoint & p, GObjWPT ** gobj, int radius) const {
  Gtk::TreeNodeChildren::const_iterator i;
  for (i  = store->children().begin();
       i != store->children().end(); i++){
    if (!(*i)[columns.checked]) continue;
...
WptLL::find_wpts(const iRect & r) const {
...
bool
WptLL::upd_wp (Workplane & wp, int & d) const {
  bool ret=false;
  Gtk::TreeNodeChildren::const_iterator i;
  for (i = store->children().begin();
       i != store->children().end(); i++){
...
bool
WptLL::upd_comm(GObjWPT * sel_gobj, bool dir){
  bool ret=false;
  Gtk::TreeNodeChildren::const_iterator i;
  for (i = store->children().begin();
       i != store->children().end(); i++){
