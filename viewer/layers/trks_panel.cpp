...
void
TrkLL::add_gobj (const boost::shared_ptr<GObjTRK> layer,
...
void
TrkLL::remove_gobj (const GObjTRK * L){
...
void
TrkLL::get_data(geo_data & world, bool visible) const {
  Gtk::TreeNodeChildren::const_iterator i;
  for (i  = store->children().begin();
       i != store->children().end(); i++){
...
TrkLL::find_tpts(const iRect & r) const {
...
int
TrkLL::find_tpt(const iPoint & p, GObjTRK ** gobj,
...
bool
TrkLL::upd_wp (Workplane & wp, int & d) const {
  bool ret=false;
  Gtk::TreeNodeChildren::const_iterator i;
  for (i = store->children().begin();
       i != store->children().end(); i++){
...
bool
TrkLL::upd_comm(GObjTRK * sel_gobj, bool dir){
  bool ret=false;
  Gtk::TreeNodeChildren::const_iterator i;
  for (i = store->children().begin();
       i != store->children().end(); i++){
