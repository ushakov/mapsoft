...
const bool mp_text_labels=true;
...
struct world : std::list<object> {
  ...
  double dist_pt(const dPoint & p, double lat){ // approximate distance in m
    double dx = p.x * cos(M_PI/180.0 * lat);
    double dy = p.y;
    return 6380000 * sqrt(dx*dx+dy*dy) * M_PI/180;
  }
  double dist_pt_pt(const dPoint & p1, const dPoint & p2){ // approximate distance in m
    return dist_pt(p2-p1, (p2.y+p1.y)/2.0);
  }
  dPoint v_cm2deg(const dPoint & p, const double lat){ // convert shift
    dPoint ret(p*rscale/100.0/6380000.0 * 180/M_PI);
    ret.x /= cos(M_PI/180.0 * lat);
    return ret;
  }
  // Find point n in the line l which is nearest to the point p. Return destance in cm
  double dist_pt_l(const dPoint & p, const object & l, dPoint & n){
    double ret = 1e99;
    for (object::const_iterator i=l.begin(); i!=l.end(); i++){
  ...
  */
  ...
  // add objects and labels from fig
  ...
  // put objects to referenced fig
  int
  put_to_fig(fig::fig_world & F){
